...
function router(request, response) {
    ...
    if ((request.url === "/upack/feedName/download/bower/packageName/version"
        || request.url === "/upack/feedName/download/bower/packageName/1.1.1")) {
        // Download
        responseToDownload(response);
    } else if (request.url === "/upack/feedName/download/bower/packageName/version.WrongSize") {
        // Download with file size error
        responseToDownloadWrongSize(response);
    } else if (request.url === "/upack/feedName/download/bower/packageName/version.BadHtmlCode") {
        // Download with file size error
        ...
        if (data.API_Key === share.testApiKey) {
            if ((partial === "/api/json/ProGetPackages_GetPackageVersions"
