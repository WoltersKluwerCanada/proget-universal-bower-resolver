...
class ProgetApi{
    constructor(config){
        ...
        if(this.registries.length > 0){
            ...
            this.registries.forEach(function(registry){
                let apiEntry = config.proget.apiKeyMapping.find(function(api){
    ...
    sendRequest (source, apiMethod) {
        ...
        return new Promise((resolve, reject) => {
            ...
            let registry = this.registries.find(function(registry){
                return api.urlToKey(registry.server) == api.urlToKey(rUrl[0]);
            });
            // Prepare the request
            let params = {
    ...
    isSuportedServer(source){
        ...
        return this.registries.some(function(registry){
            return api.urlToKey(source).indexOf(api.urlToKey(`${registry.server}/?${registry.feed}`)) > -1;
        });
    }
    ...
    fillCache(packageName){
        ...
        return Promise.mapSeries(
                ...
                .map(function(url){
                    return api.getPackagesSingle(url).then(function (sourcePackages) {
                        if(sourcePackages.length > 0){
                            if(api.cachedPackages[packageName] === undefined)
                                api.cachedPackages[packageName] = {};
                            let formattedUrl = url.replace("?", "/?")
                            api.cachedPackages[packageName][formattedUrl] = sourcePackages;
                            return formattedUrl;
                        }
                        return null;
                    });
            ...
            function(validSource){
                if(api.cachedPackages[packageName] !== undefined && api.cachedPackages[packageName].defaultSource == null && validSource != null)
                    api.cachedPackages[packageName].defaultSource = validSource;
            });
    }
