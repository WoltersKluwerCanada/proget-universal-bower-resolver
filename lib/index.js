    ...
    return {
        ...
        match: function(source) {
            if (source.indexOf(bower.config.proget.server) > -1) {
                ...
                return request(utils.registryParser(source, bower.config), "ProGetPackages_GetPackages", bower.config).then(
                    (response) => {
                        try {
         ...
         */
        releases(source) {
            return request(source, "ProGetPackages_GetPackageVersions", bower.config).then((response) => {
        ...
        fetch(endpoint, cached) {
            if (cached.version !== endpoint.target) {
                return request(endpoint.source, "Feeds_GetFeed", bower.config).then((feed) => {
